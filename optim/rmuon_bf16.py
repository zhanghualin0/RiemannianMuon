...
__all__ = ["RiemannianMuon", "RiemannianMuonWithAuxAdam", "HybridStiefel"]


class HybridStiefel(EuclideanStiefel):
    """
    Stiefel manifold with hybrid bfloat16/float32 approach.
    
    Keeps parameters in bfloat16 for memory efficiency, but casts to float32
    for exact linalg operations (QR/SVD), then casts back to bfloat16.
    """
    ...
    def retr(self, x: torch.Tensor, u: torch.Tensor) -> torch.Tensor:
        """
        Retraction using QR decomposition with dtype casting.
        
        Parameters
        ----------
        x : torch.Tensor
            Current point on manifold (bfloat16)
        u : torch.Tensor  
            Tangent vector (bfloat16)
            
        Returns
        -------
        torch.Tensor
            New point on manifold after retraction (bfloat16)
        """
        # Cast to float32 for QR decomposition
        x_f32, u_f32 = x.float(), u.float()
        x_plus_u_f32 = x_f32 + u_f32
        
        # QR decomposition in float32
        q, r = torch.linalg.qr(x_plus_u_f32)
        
        # Sign correction using geoopt's approach
        unflip = torch.diag(r).sign().add(0.5).sign()
        q_corrected = q * unflip[..., None, :]
        
        # Cast back to original dtype
        return q_corrected.to(x.dtype)
    
    def projx(self, x: torch.Tensor) -> torch.Tensor:
        """
        Project matrix onto Stiefel manifold using SVD with dtype casting.
        
        Parameters
        ----------
        x : torch.Tensor
            Input matrix to project (bfloat16)
            
        Returns
        -------
        torch.Tensor
            Projected matrix on Stiefel manifold (bfloat16)
        """
        # Cast to float32 for SVD decomposition
        x_f32 = x.float()
        
        # SVD decomposition in float32
        ...
        # Cast back to original dtype
        return orthogonal_f32.to(x.dtype)
